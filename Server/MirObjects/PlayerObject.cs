...
namespace Server.MirObjects
{
    public class PlayerObject : HumanObject
    {
        ...
        public const long TurnDelay = 350, HarvestDelay = 350, MovementDelay = 2000;
        public long ChatTime, ShoutTime, RestedTime, MovementTime;

        public byte ChatTick;

        protected int _restedCounter;
        ...
        public override void GainExp(uint amount)
        {
            ...
            if (MyGuild != null && MyGuild.Name != Settings.PalaceGuild)
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "RECALLMOB":
                        ...
                        if (!IsGM && (Pets.Count(t => !t.Dead) >= Globals.MaxPets)) return;
                    ...
                    case "ALLOWGROUP":
                        ...
                        ReceiveChat($"Allow Group: {AllowGroup}", ChatType.Group);
                        break;
        ...
        public override Packet GetInfo()
        {
            ...
            return new S.ObjectPlayer
            {
                ...
                Dead = Dead,
                Hidden = Hidden,
                Effect = HasBuff(BuffType.MagicShield, out _) ? SpellEffect.MagicShieldUp : SpellEffect.None,
        ...
        public void SplitItem(MirGridType grid, ulong id, ushort count)
        {
            ...
            if (grid == MirGridType.Inventory && (temp.Info.Type == ItemType.Potion || temp.Info.Type == ItemType.Scroll || (temp.Info.Type == ItemType.Script && temp.Info.Effect == 1)))
            {
        ...
        public void CombineItem(MirGridType grid, ulong fromID, ulong toID)
        {
            ...
            bool canRepair = false, canUpgrade = false, canSeal = false;
        ...
        public void SellItem(ulong uniqueID, ushort count)
        {
            ...
            if (NPCPage == null || !(String.Equals(NPCPage.Key, NPCScript.BuyKey, StringComparison.CurrentCultureIgnoreCase) || String.Equals(NPCPage.Key, NPCScript.SellKey, StringComparison.CurrentCultureIgnoreCase)))
            {
                Enqueue(p);
                return;
            }
        ...
        public bool CreateGuild(string guildName)
        {
            ...
            if (!Info.AccountInfo.AdminAccount && String.Equals(guildName, Settings.PalaceGuild, StringComparison.OrdinalIgnoreCase))
            {
