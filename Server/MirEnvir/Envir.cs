...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        private long warTime, guildTime, auctionTime, spawnTime, robotTime, timerTime;
        ...
        public void LoadBanIP()
        {
            ...
            var path = Path.Combine(Settings.EnvirPath, "BannedIPs.txt");
            ...
            {
                BanIPList.Add(lines[i]);
            }
        }

        public void SaveBanIP()
        {
            string path = Path.Combine(Settings.EnvirPath, "BannedIPs.txt");
            ...
            File.WriteAllLines(path, BanIPList);
        }
        ...
        public void Login(ClientPackets.Login p, MirConnection c)
        {
            ...
            int connectIPCount = 0;
            foreach (MirConnection con in Connections)
            {
                if (c.IPAddress == con.IPAddress)
                    connectIPCount++;

                if (connectIPCount > Settings.MaxIP)
                {
                    c.Enqueue(new ServerPackets.Login { Result = 5 });
                    return;
                }
            ...
            foreach (string banip in BanIPList)
            {
                if (c.IPAddress == banip)
                {
                    c.Enqueue(new ServerPackets.Login { Result = 6 });
                    return;
                }
            }
            lock (AccountLock)
